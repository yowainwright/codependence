...
const updatePEP621AndUvDependencyGroupsOriginal = `[project]
dependencies = [
  "requests>=2.31.0",
  "flask==3.0.0",
]
...
`;
      ...
      assert.match(content, /# Generated by uv/);
      assert.match(content, /requests==2\.31\.0/);
      assert.match(content, /--hash=sha256:abc123/);
      assert.match(content, /flask>=2\.1\.0 ; python_version >= "3\.11"/);
      assert.match(content, /-r shared-requirements\.txt/);
...
`;
      ...
      assert.match(content, /\[tool\.poetry\.dependencies\]/);
      assert.match(content, /python = "\^3\.8"/);
      assert.match(content, /requests = "\^2\.31\.0"/);
      assert.match(content, /flask = "~2\.0\.0"/);
      assert.match(content, /old-package = "\^1\.0\.0"/);
      assert.match(content, /\[tool\.poetry\.dev-dependencies\]/);
    });
    ...
    test("should update PEP 621 and uv dependency groups", async () => {
      const pyprojectPath = join(tmpDir, "pyproject.toml");
      writeFileSync(pyprojectPath, updatePEP621AndUvDependencyGroupsOriginal);
      ...
      assert.match(content, /"requests>=2\.32\.0"/);
      assert.match(content, /"flask==3\.0\.0"/);
      assert.match(content, /"mkdocs>=1\.6\.0"/);
      assert.match(content, /"pytest>=8\.1\.0"/);
      assert.match(content, /"pytest-benchmark>=4\.1\.0"/);
    });
...
`;
      ...
      assert.match(content, /"requests\[security\]>=2\.32\.0"/);
      assert.match(content, /"boto3>=1\.34\.0"/);
    });
  });
...
`;
      ...
      assert.match(content, /\[packages\]/);
      assert.match(content, /requests = "==2\.31\.0"/);
      assert.match(content, /flask = ">=2\.0\.0"/);
      assert.doesNotMatch(content, /old-package/);
      assert.match(content, /\[dev-packages\]/);
    });
  });
...
`;
      ...
      assert.match(content, /  - numpy=1\.25\.0 # keep comment/);
      assert.match(content, /  - pandas>=2\.1\.0/);
      assert.match(content, /  - scipy=1\.10\.0/);
      assert.match(content, /      - requests==2\.31\.0/);
      assert.match(content, /variables:\n  EXAMPLE: true/);
    });
  });
