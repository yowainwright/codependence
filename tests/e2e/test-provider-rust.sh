...
write_rust_dependencies_codependencerc() {
  cat >"$WORK_DIR/.codependencerc" <<'JSON'
{"mode":"verbose","codependencies":[{"serde":"1.0.210"},{"tokio":"1.40.0"},{"serde-json":"1.0.145"},{"pretty-assertions":"1.4.1"},{"cc":"1.1.30"},{"libc":"0.2.155"}]}
JSON
}
...
test_rust_dependencies() {
  ...
  assert_file_equals "$FIXTURE_DIR/expected/rust-Cargo.toml.expected" "$WORK_DIR/Cargo.toml" "rust update matches golden output"
  ...
  assert_file_contains "$WORK_DIR/Cargo.toml" 'serde_json_renamed = { package = "serde_json", version = "1.0.145" }' "renamed dependency updated"
  ...
  assert_file_unchanged_after_update "$WORK_DIR" "$WORK_DIR/Cargo.toml" "rust update is idempotent"
}
