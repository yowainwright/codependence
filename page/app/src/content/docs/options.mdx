...
The primary config shape is a non-empty `targets` array. Every target requires a `manager` plus `codependencies`, `permissive`, or `mode`, and can define its own `files`, `rootDir`, `ignore`, and `level`.
