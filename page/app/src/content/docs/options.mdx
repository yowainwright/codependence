...
### `permissive`: `boolean`
**Recommended mode.** Update all dependencies to latest except those in `codependencies`. Default: `true`

### `mode`: `string`
Controls which packages are checked. `verbose`: only listed packages; `precise`: all except listed. Default: `precise`
