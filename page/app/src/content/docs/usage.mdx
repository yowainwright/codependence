...
```bash
nub run skills:install
nub run skills:install:codex
nub run skills:install:claude
```
...
# Generate split dependency update workflows
...
`codependence init actions` generates separate Node, Python, Go, Rust, Docker,
and GitHub Actions workflows from configured manager targets.
